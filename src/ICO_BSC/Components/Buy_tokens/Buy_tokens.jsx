...
function Buy_tokens(props, connect) {
  ...
  const BuyARCWithUSDC = async (data) => {
    try {
      ...
      let User_Token_Balance = await TokenContractOf.methods
        .balanceOf(acc)
        .call();
      let BalanceOf = web3.utils.fromWei(User_Token_Balance.toString());
      ...
      web3.eth.getBalance(ico_contract.toString(), function (err, result) {
        if (err) {
          ...
          console.log(value);
          console.log("BalanceOf", Number(BalanceOf) < Number(data));
          if (Number(data) > Number(BalanceOf)) {
            setError(
  ...
  const buyWARCwithBUSD = async (data) => {
    try {
      ...
      accounts = await web3.eth.getAccounts();
      let ICO_ContractOf = new web3.eth.Contract(contractabi, ico_contract);
      let TokenContractOf = new web3.eth.Contract(BUSDabi, BUSD_contract);

      let value = web3.utils.toWei(data.toString());
      let getValue = await ICO_ContractOf.methods
        .getARCvalueperBUSD(value.toString())
        .call();
      setGetEthIput(data);
      value = web3.utils.fromWei(getValue.toString());
      ...
      let User_Token_Balance = await TokenContractOf.methods
        .balanceOf(acc)
        .call();
      BalanceOf = web3.utils.fromWei(User_Token_Balance.toString());
      ...
      web3.eth.getBalance(ico_contract.toString(), function (err, result) {
        if (err) {
          ...
          setGetEthValue(value);
          console.log(value);
          if (BalanceOf < Number(data)) {
            setError(
              "Oops! It looks like you don't have enough BUSD. Please reduce the amount of BUSD and try again."
            );
            setGetEthValue(value);
          }else{
            setError(
      ...
      >
        <Modal.Body className="bg_model">
          ...
          <p className="scond_text_success text-center">
            ...
            <a target="_blank" href={`https://app.archiescan.io/tx/${getRes}`}>
